...
1. **Setup do Projeto**: Repositório React + Vite + Tailwind (Feito).
2. **Database Layer**: Configurado PGlite local via Drizzle ORM (Feito).
3. **Prototipar a Tela de Input**: Timeline e Vídeo Player responsivo (Feito).

---

## Fase 2: Módulo de Visão Computacional (OpenCV.js)

O grande diferencial do TeamTrack é o pré-processamento local de vídeo para facilitar o scout. Para isso, usaremos `opencv.js` importado dinamicamente dentro de um Web Worker.

### Arquitetura de Visão Computacional Local

1. **Vídeo Source**: O `ReactPlayer` vai tocar o vídeo.
2. **Frame Extraction**: Usaremos um `<canvas>` invisível no componente `VideoAnalysis` que a cada `requestAnimationFrame` faz um `ctx.drawImage(videoElement)`.
3. **Worker Comms**: Enviaremos o `ImageData` (via transferência de buffer ou `OffscreenCanvas`) para o `cv.worker.ts`.
4. **Processamento OpenCV.js**:
    * **Carregamento**: Usaremos as bibliotecas pré-compiladas WASM do OpenCV. O worker vai aguardar o OpenCV inicializar antes de aceitar frames.
    * **Detecção de Movimento (Rallies)**: Usaremos `cv.createBackgroundSubtractorMOG2()` para ignorar tempo morto. Se a quantidade de "pixels brancos" na máscara for alta, consideramos que a bola está em jogo.
    * **Rastreamento da Bola**: Em frames de "Rally ativo", usaremos conversão HSV + `cv.inRange` (baseado na cor amarela/azul clássica da bola Mikasa) + `cv.findContours` para achar as coordenadas prováveis da bola.
5. **Calibração / Homografia**:
    * A partir dos 4 pontos selecionados pelo usuário na tela, calculamos `cv.getPerspectiveTransform()`.
    * Isso nos permite converter a posição XY do pixel da bola no vídeo para medidas reais em "metros" na quadra (ex: 9x18m).

### Desafios / Pontos de Alerta (User Review)

> [!WARNING]
> Carregar e processar o `opencv.js` puro em WASM consume bastante memória do navegador abertos. Pode ser fatal em dispositivos de RAM baixa.
> **Mitigação**: O OpenCV WASM tem por volta de 8MB-10MB. Enviaremos as resoluções de frame ao worker redimensionadas (ex: 480p ou menos) para evitar gargalo de processamento em tempo real.

### Verificação

#### Testes Manuais

1. Rodar a aplicação com `npm run dev`.
2. Carregar um vídeo de teste limpo e estático no painel.
3. Calibrar a quadra.
4. Pressionar "Iniciar Análise Automática".
5. Visualizar a UI exibindo o tempo total extraído de "Tempo Morto vs Tempo Ativo" e as anotações visuais da bola caindo provisoriamente na tela.
